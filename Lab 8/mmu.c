...
void allocate_memory(list_t *freelist, list_t *alloclist, int pid, int blocksize, int policy) {
    ...
    while (current != NULL) {
        ...
        if (blk_size >= blocksize) {
            // Block found, allocate memory here
            allocated_block = malloc(sizeof(block_t));
            allocated_block->start = blk->start;
            allocated_block->end = blk->start + blocksize - 1;
            ...
            // Add the allocated block to the alloc list in ascending order by address
            list_add_ascending_by_address(alloclist, allocated_block);

            // Handle leftover fragment if there's any
            if (blk_size > blocksize) {
                ...
                // Add the fragment back to the freelist based on the policy
                if (policy == 1) {
                    list_add_to_back(freelist, fragment); // FIFO policy
                } else if (policy == 2) {
                    list_add_ascending_by_blocksize(freelist, fragment); // Best fit policy
                } else if (policy == 3) {
                    list_add_descending_by_blocksize(freelist, fragment); // Worst fit policy
                }
            }
            return;
        }
    ...
    // If no block large enough is found, print error
...
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy) {
    ...
    while (current != NULL) {
        block_t *blk = current->blk;
        if (blk->pid == pid) {
            // Block found, deallocate it
            blk->pid = 0;

            // Remove the block from the alloc list
            if (prev == NULL) {
            ...
            // Add the block back to the freelist based on the policy
            if (policy == 1) {
                list_add_to_back(freelist, blk); // FIFO policy
            } else if (policy == 2) {
                list_add_ascending_by_blocksize(freelist, blk); // Best fit policy
            } else if (policy == 3) {
                list_add_descending_by_blocksize(freelist, blk); // Worst fit policy
            }
            return;
        }
    ...
    // If the block is not found, print error
